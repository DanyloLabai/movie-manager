...
import { JOURNEY_VIEWBOX } from '../../data/journeyLotr';
import JourneyMap from './JourneyMap';
import { GOLD } from './JourneyPin';

// The map is laid out at a fixed size and then scaled/translated as a whole,
// like movie-frontend's TransformWrapper (react-zoom-pan-pinch).
const MAP_W = 1400;
...
// Full-screen pannable/zoomable map. No gesture-handler/reanimated in this
// app, so pinch + drag are built on RN's PanResponder driving Animated values
// (no re-render per frame); +/−/reset buttons cover the case where a gesture
// isn't convenient, same as web.
export default function JourneyMapModal({ visible, totalCount, onClose, youLabel }: JourneyMapModalProps) {
  ...
  // Page-space position of the viewport's top-left, so touch pageX/pageY can
  // be converted into viewport coordinates for focal-point zooming.
  ...
  const apply = useCallback((next: Transform) => {
    if (!viewport) return;
    const { min, max } = { min: Math.min(fitScale.current, 0.4), max: Math.max(1.6, fitScale.current * 6) };
    const scale = Math.min(Math.max(next.scale, min), max);
    // Keep at least a quarter of the map overlapping the viewport on each axis.
    const half = { x: (scale * MAP_W) / 2, y: (scale * MAP_H) / 2 };
    const cx = Math.min(Math.max(MAP_W / 2 + next.tx, viewport.w * 0.25 - half.x), viewport.w * 0.75 + half.x);
    const cy = Math.min(Math.max(MAP_H / 2 + next.ty, viewport.h * 0.25 - half.y), viewport.h * 0.75 + half.y);
  ...
  // Zooming about a focal point f (viewport coords) keeps the map point under
  // it fixed: screen(p) = centre + t + s·(p − centre), so solve for t'.
  const zoomAbout = useCallback(
    (from: Transform, scale: number, fx: number, fy: number): Transform => {
      const ratio = scale / from.scale;
      const ox = fx - MAP_W / 2;
      const oy = fy - MAP_H / 2;
      return { scale, tx: ox - ratio * (ox - from.tx), ty: oy - ratio * (oy - from.ty) };
    },
    [],
  );
  ...
  const reset = useCallback(() => {
    ...
    setZoomedIn(false);
    apply({ scale: fitScale.current, tx: viewport.w / 2 - MAP_W / 2, ty: viewport.h / 2 - MAP_H / 2 });
  }, [viewport, apply]);
      ...
      PanResponder.create({
        ...
        onPanResponderMove: (e) => {
          ...
          if (touches.length >= 2) {
            ...
            // Two-finger drag pans while pinching.
            apply({
              scale: zoomed.scale,
  ...
  const zoomByButton = (factor: number) => {
    if (!viewport) return;
    apply(zoomAbout(current.current, current.current.scale * factor, viewport.w / 2, viewport.h / 2));
  };
        ...
        >
          ...
          >
            <JourneyMap
              ...
              zoomedIn={zoomedIn}
              youLabel={youLabel}
              onStopClick={(id) => setOpenStopId((prev) => (prev === id ? null : id))}
            />
          </Animated.View>
        </View>

        <Pressable style={[styles.roundButton, styles.closeButton]} onPress={onClose} hitSlop={8}>
          <Ionicons name="close" size={20} color="#8f8574" />
        </Pressable>

        <View style={styles.controls}>
          <Pressable style={styles.roundButton} onPress={() => zoomByButton(BUTTON_ZOOM_STEP)}>
            <Text style={styles.controlGlyph}>+</Text>
          </Pressable>
          <Pressable style={styles.roundButton} onPress={() => zoomByButton(1 / BUTTON_ZOOM_STEP)}>
            <Text style={styles.controlGlyph}>−</Text>
          </Pressable>
          <Pressable style={styles.roundButton} onPress={reset}>
            <Text style={styles.resetText}>{t('journey.zoomReset').toUpperCase()}</Text>
          </Pressable>
        </View>
      </SafeAreaView>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  // Fixed-size map anchored top-left; the transform does the rest.
  content: { position: 'absolute', left: 0, top: 0, width: MAP_W, height: MAP_H },
  roundButton: {
    width: 36,
    height: 36,
    borderRadius: 18,
  ...
  closeButton: { position: 'absolute', top: 12, right: 14 },
  controls: { position: 'absolute', right: 14, bottom: 32, gap: 6 },
  controlGlyph: { color: GOLD, fontSize: 20, fontWeight: '700', lineHeight: 22 },
