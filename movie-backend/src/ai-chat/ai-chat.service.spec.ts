...
describe('AiChatService', () => {
  ...
  describe('searchMovieByDescription', () => {
    ...
    it('returns the Gemini response directly when the primary provider succeeds (happy path)', async () => {
    ...
    it('falls back to Groq when the primary Gemini call throws, and returns its response', async () => {
      mockGenerateObject
        .mockRejectedValueOnce(new Error('Gemini is down'))
      ...
      // Only the successful (failover) call is logged- the failed Gemini attempt is not.
      expect(mockAiUsageLogService.logUsage).toHaveBeenCalledTimes(1);
      expect(mockAiUsageLogService.logUsage).toHaveBeenCalledWith(
        expect.objectContaining({
          userId,
    ...
    it('falls back to DeepSeek when both Gemini and Groq throw, and returns its response', async () => {
      mockGenerateObject
        .mockRejectedValueOnce(new Error('Gemini is down'))
    ...
    it('throws InternalServerErrorException without crashing when Gemini, Groq and DeepSeek all fail', async () => {
      mockGenerateObject
        .mockRejectedValueOnce(new Error('Gemini is down'))
        .mockRejectedValueOnce(new Error('Groq is down too'))
        .mockRejectedValueOnce(new Error('DeepSeek is down too'));
    ...
    describe('usage logging (feeds AiDailyLimitGuard/AiUsageLogService)', () => {
      it('logs provider=gemini/wasFailover=false with the token count reported by generateObject on success', async () => {
        mockGenerateObject.mockResolvedValueOnce({
          object: baseAiObject,
      ...
      it('logs provider=groq/wasFailover=true when Gemini failed over to Groq', async () => {
        mockGenerateObject
          .mockRejectedValueOnce(new Error('Gemini is down'))
          .mockResolvedValueOnce({
            object: baseAiObject,
